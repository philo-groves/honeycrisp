...
export function createResearchSystemPrompt(
  ...
  const systemPrompt = [
    ...
    ...(options.hasCollaborationTools ? [
      ...
      "When you delegate, wait for requested subagent results before concluding and avoid assigning overlapping work.",
      "When you choose a breakout room, treat it as a bounded evidence cell rather than a majority vote. Preserve dissent, cite tool or artifact evidence, and let verification decide disputed claims.",
      "Use create_room for a breakout room with at least two members. Use spawn_agent for ordinary independent delegation.",
      "Breakout-room members use room_publish and room_wait: independent memos remain blind until every member submits, then bounded challenge and response phases advance only after every member participates.",
      "The lead agent cannot be a breakout-room member. For any room it creates, it reads the released packets, preserves adopted and rejected claims plus dissent, and publishes the structured outcome before its final response.",
      ...(profile?.collaboration.protocolInstructions.map((instruction) => `Profile collaboration protocol: ${instruction}`) ?? []),
      ...(collaborationRecipe ? [
        `Available profile collaboration recipe when collaboration is warranted: ${collaborationRecipe.name} (${collaborationRecipe.id}); room kind ${collaborationRecipe.roomKind}.`,
