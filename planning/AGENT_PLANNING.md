...
- [x] Remove Beale fake run engine from production code or confine it to narrow renderer/test fixtures.
- [x] Remove Beale OpenAI run engine, OpenAI tool stack, and OpenAI context compaction; keep OpenAI auth/adapter only for Beale-owned prompt planning and HackerOne import review while those remain in Beale.
- [x] Remove executor-alpha run engine after VM/sandbox removal.
- [x] Reduce `RunEngineKind` to Honeycrisp plus any explicit non-agent UI fixture mode needed for tests.
- [x] Ensure Beale's run records, model sessions, trace events, and transcripts are imported from Honeycrisp captures rather than generated by a parallel agent loop.
- [x] Update run settings defaults and UI labels to reflect Honeycrisp as the engine.
- [x] Replace Beale tool traces with Honeycrisp tool events where possible.
- [x] Add tests that starting a research session invokes Honeycrisp and imports goal/subgoal/context/memory/proof/finding state.
- [x] Run a real Beale/Honeycrisp session and verify no Beale-native agent runtime code is used.
