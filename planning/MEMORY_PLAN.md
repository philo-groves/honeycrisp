...
- Derived memory records are typed and generated by the write pipeline, but not yet persisted as first-class entities.
...
The next implementation slice should be phase 4 only:

1. Add SQLite-backed typed derived-record storage and indexes.
2. Persist records produced by the phase-3 `MemoryWritePipeline`.
3. Add claim graph edge storage for support, contradiction, refinement, supersession, and dependency links.
4. Add record read/list/update APIs with status-aware behavior.
5. Keep tombstoned records out of ordinary recall and context while preserving auditability.
...
- Existing context packets still compile successfully.
