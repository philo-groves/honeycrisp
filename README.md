...
On a real run, Honeycrisp opens the user-global database at `~/.honeycrisp/memory.sqlite`, exposes configured research tools plus a small durable knowledge graph and workspace runbook family, and runs the selected model through Pi. Records retain their workspace, session, and subject identities so one database can serve headless and host-driven research across workspaces. Session events and capture bodies use transactionally migrated, individually hashed rows so live appends do not rewrite the complete prior timeline. Core protocol reads return session metadata; bounded cursor pages serve transcripts and traces, targeted operations serve requested event details and collaboration state, and capture metadata is separate from explicit capture-detail reads. Captures reference the normalized event stream rather than embedding duplicate event histories. Structural SQLite corruption and content-hash failures are reported as distinct non-retryable protocol errors with instructions to stop writers and preserve the original database before restore or repair. Run state, transcripts, and tool traces are operational data; they are not automatically promoted into durable knowledge. The model explicitly searches and updates concise reusable nodes when the research warrants it.
...
Runbooks are workspace-scoped, revisioned Jupyter `nbformat 4` artifacts for reusable proof sequences, environment setup, diagnostics, and investigation procedures. `runbook.list`, `runbook.get`, `runbook.create`, and `runbook.append` preserve ordered markdown/code cells, while `runbook.run` executes one cell, an inclusive `startCellId`/`endCellId` range, or the complete ordered code-cell sequence through the normal `shell.run` broker and its utility controls. `runbook.get` returns stable cell IDs and indexes so a repaired late failure can resume from that cell without repeating a successful prefix. Content revisions are counted independently from runs, completed cells, and latest execution status; execution-state persistence does not create content-update events. Each cell records its latest status, duration, exit code, and bounded output. In Auto-Review, commands classified as proofing require trusted runbook execution context and are denied when issued directly through `shell.run`.

## Client protocol

`honeycrisp protocol describe --json` advertises the versioned protocol operations available to Beale and other clients. In addition to revisioned sessions and knowledge operations, protocol v1 owns auxiliary model-job routing and completion semantics, provider defaults, source discovery and repository materialization, Agent Plugin registry/runtime assembly, and retained workspace maintenance. Contract v4 keeps `session get` and session catalogs metadata-only; clients use `session get-update` or `session events` with bounded cursors, `session event-details` for exact records, `session collaboration` for current room state, and `session captures`/`session capture` for capture metadata and explicitly requested detail. CLI calls use one JSON success/error envelope and file-backed JSON inputs; `--request-id <id>` is echoed in both success and failure envelopes. Live session events and controls use the authenticated loopback WebSocket transport with the same protocol version, request correlation, and error detail DTO. Clients render results, inject host credentials and client-specific built-ins, and apply UI authorization gates without reimplementing harness behavior or opening Honeycrisp's database.
...
Under the bundled security profile, durable knowledge uses `asset`, `bug`, `invariant`, `mitigation`, `source`, `sink`, `hypothesis`, `primitive`, `chain`, `procedure`, and `trajectory` nodes, plus directed relationships, tags, asset links, and lightweight evidence references. A `hypothesis` is a specific, testable but unproven proposition: keep it draft or suspected while active, reject it when disproven, and reclassify it as a primitive or chain when proven. `evidence` and `finding` are not node types: evidence remains attached references, proven mechanisms remain primitives or chains, and a separate canonical finding record tracks the operational vulnerability lifecycle without overloading knowledge taxonomy. Finding transitions are revision-guarded and ordered; observation needs direct referenced evidence, reproduction needs the run ID of a successful ledger-backed runbook execution, verification must originate outside the finding's session, and reporting/disclosure require their exact durable references. Other profiles may define entirely different stable IDs and display names. Saves are additive; exact corrections require the current node revision. Transcripts, task narration, and bulk tool output do not belong in the graph.
...
Honeycrisp builds a campaign projection from authorized assets, durable memory, canonical findings, runbooks, and reports. It identifies uncovered assets, unsupported claims, hypotheses that have not crossed observation, missing reproduction or independent verification, missing reports, stale findings, and explicit contradictory relationships. The prioritized gaps and typed momentum are injected into every security-research run so the agent starts from recorded territory and pursues a discriminating next action instead of redoing orientation.
...
Honeycrisp replies with `server.hello`, streams existing live event objects inside `session.event`, and reports accepted or rejected controls as `control.received` agent events. Invalid messages receive `protocol.error` with the shared `{code,message,retryable}` error detail and the request ID when one was recoverable. One authenticated client is allowed per run. Messages are capped at 1 MiB, and disconnecting that client stops the active run. The bearer token is never included in the bootstrap record. TypeScript clients import all CLI and WebSocket DTOs, factories, decoders, and constants from `honeycrisp/protocol`; `honeycrisp/websocket-protocol` remains an alias for v1 compatibility.
...
Honeycrisp's transport-neutral client contract is versioned separately from individual data schemas. `honeycrisp protocol describe --json --request-id <id>` returns the transport protocol, contract version, executing runtime build identity, supported schema versions and capabilities, operations, CLI framing, correlation, error semantics, and WebSocket capabilities in a correlated success envelope. WebSocket `server.hello` carries the same contract and schema identity. TypeScript clients import the shared CLI and WebSocket contract from `honeycrisp/protocol`.
