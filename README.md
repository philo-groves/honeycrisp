...
Durable graph nodes carry a memory tier and their origin context. `session` nodes are visible only to one research session, `workspace` nodes are reusable across sessions for one workspace, and `subject` nodes can be recalled across workspaces with the same owner or subject. `memory.search` queries workspace memory by default and accepts an explicit session or subject tier override. `memory.save` defaults to workspace and refines an exact visible type-and-title identity in place, retaining its existing tier instead of creating a cross-tier copy. Visibility is enforced from the active workspace context even though records share one database.
